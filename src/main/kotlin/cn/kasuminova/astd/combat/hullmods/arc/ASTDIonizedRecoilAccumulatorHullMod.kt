...
class ASTDIonizedRecoilAccumulatorHullMod : BaseHullMod() {
    ...
    companion object {
        ...
        private const val HARD_FLUX_CONVERT_FRACTION = 0.005f
    ...
    private class IonizedRecoilListener(private val ship: ShipAPI) : DamageTakenModifier, AdvanceableListener {
        ...
        override fun modifyDamageTaken(param: Any?, target: CombatEntityAPI?, damage: DamageAPI?, point: Vector2f?, shieldHit: Boolean): String? {
            ...
            val converted = convertHardFlux()
            if (converted <= 1f) return null

            cooldown = COOLDOWN_SECONDS
            val hardFluxLevel = hardFluxLevel()
            discharge(point, converted, hardFluxLevel)
            return null
        }
        ...
        private fun convertHardFlux(): Float {
            val tracker = ship.fluxTracker
            val converted = (tracker.hardFlux * HARD_FLUX_CONVERT_FRACTION).coerceAtLeast(0f)
        ...
        private fun discharge(from: Vector2f, converted: Float, hardFluxLevel: Float) {
            val engine = Global.getCombatEngine() ?: return
            val target = chooseTarget(engine)
            ...
            if (target != null) {
                ...
                val arc = if (Math.random().toFloat() < pierceChance) {
                    engine.spawnEmpArcPierceShields(ship, from, ship, target, DamageType.ENERGY, damage, emp, RANGE, null, 9f, ARC_FRINGE, ARC_CORE, params)
                } else {
                    engine.spawnEmpArc(ship, from, ship, target, DamageType.ENERGY, damage, emp, RANGE, null, 9f, ARC_FRINGE, ARC_CORE, params)
                }
                arc.setCoreWidthOverride(3.0f)
                ...
                engine.spawnEmpArcVisual(from, ship, to, ship, 6f, ARC_FRINGE, ARC_CORE, arcParams()).setSingleFlickerMode(true)
            }
        }
        ...
        private fun chooseTarget(engine: CombatEngineAPI): ShipAPI? {
