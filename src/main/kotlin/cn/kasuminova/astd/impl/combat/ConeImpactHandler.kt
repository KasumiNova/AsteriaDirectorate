...
object ConeImpactHandler {
    ...
    internal const val DIST_EPS = 0.5f

    /** 角度/距离比较的浮点容忍（度/su）：恰在边界的目标判定为纳入（含边界语义）。 */
    internal const val ANGLE_EPS = 1e-3f
    internal const val RANGE_EPS = 1e-3f
     ...
     */
    internal fun surfacePoint(origin: Vector2f, targetLoc: Vector2f, collisionRadius: Float): Vector2f {
        val dx = targetLoc.x - origin.x
        val dy = targetLoc.y - origin.y
        val dist = sqrt(dx * dx + dy * dy)
