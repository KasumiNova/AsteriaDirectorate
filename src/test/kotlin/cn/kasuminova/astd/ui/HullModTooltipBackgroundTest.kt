...
class HullModTooltipBackgroundTest {
    ...
    fun `hullmod tooltip renderer uses lattice pulse background instead of nebula`() {
        val rendererPath = Path.of(
            "src/main/kotlin/cn/kasuminova/astd/combat/hullmods/base/ASTDHullModTooltipRenderer.kt",
        )
        val renderer = Files.readString(rendererPath)
        assertTrue(
    ...
    fun `lattice pulse background mirrors tooltip editor shader constants`() {
        val sourcePath = Path.of(
            "src/main/kotlin/cn/kasuminova/astd/ui/effect/ASTDLatticePulseTooltipBackground.kt",
        )
    ...
    fun `arc flare mode hullmod names match tooltip exports`() {
        val i18nPath = Path.of("ss-csv/src/main/resources/i18n/zh-cn.properties")
        val i18n = Files.readString(i18nPath)
