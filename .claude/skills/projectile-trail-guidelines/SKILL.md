...
- **带体头离散落后（「暂停后不同步/跳变」的实锤真相，2026-09 trail_pause_probe 三帧对照定论）**：节点记录按 `BUtil_GLImpl.advanceTimer` 的 `getTrailSystemNodesRecordsCycle()` 门控（LunaLib `BUtil_TrailSystemQuality`：NORMAL 30Hz / HIGH 60Hz / ULTRA 144Hz），每个 cycle 只在 `computeData` 写**一个**节点、渲染头=最后记录节点——cycle 间拖尾头停在上一节点、弹体继续飞，**aod7 2880su/s 在 30Hz 下每节点跨 96su**，带头阶梯跳动；暂停把滞后相位定格、恢复时下一 tick 把头前甩 96su，肉眼即「跳变」。原版螺栓逐帧渲染与 MagicTrail 逐帧采样无此现象，故只有走 Static Trail 的 ASTD 弹体出现。**暂停/恢复本身对两代拖尾后端都无渲染腐化**（对照组 `-Dastd.tpp.pauseSeconds=0` 直通复现相同画面序列，SSOptimizer 嫌疑已证伪：原版环境+RenderThread 开关均复现/无关）。ASTD 侧缓解（已落地）：recede 上限规则 `recede ≤ headLead + 弹体spec.length/2 − speed/30`，保证最坏 cadence 相位下拖尾头仍藏在原版螺栓覆盖区（aod7：90→35）。根治需上游：fill 头节点用 tracker 实时位置外插，或提供逐帧记录模式。
- **「暂停后变亮/贴图突变」的另一假象源（首轮探针教训）**：原版 `TrailExtender` 出生伸入——螺栓亮度 = `(1-progress) × distanceRatio`，distanceRatio 要飞满 `proj.length`（aod7=138su）才到 1；弹速被外力压低时（如探针 fallback 的取景曲线对齐 340su/s）伸入期拉长到肉眼可见，恰好跨暂停窗口时易被误读为「暂停导致突变」。实弹速下 0.05s 内完成，无感知。
