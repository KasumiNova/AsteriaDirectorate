  ...
  return (
    <aside className="entity-inspector">
      <ParamGroup
        ...
        id={trail.id}
        badge="Runtime"
        description="拖尾本体、颜色、材质、噪声、闪烁与 ribbon 装饰会进入游戏导出。"
      >
        ...
        <div className="param-subgroup">
          <div className="panel-inline-head">
            <span className="param-subgroup-title">Ribbon Decorations</span>
            <button type="button" onClick={addRibbonDecoration}>Add Ribbon</button>
          </div>
          {trail.ribbonDecorations.map((ribbon, ribbonIndex) => (
            <section key={ribbon.id} className="param-nested-group">
              <div className="panel-inline-head">
                <strong>{ribbon.id}</strong>
                <div className="button-row compact">
                  <button type="button" onClick={() => updateRibbonDecoration(ribbonIndex, { enabled: !ribbon.enabled })}>
                    {ribbon.enabled ? 'Disable' : 'Enable'}
                  </button>
                  <button type="button" onClick={() => removeRibbonDecoration(ribbonIndex)}>Remove</button>
                </div>
              </div>
              <BoolRow
                label="enabled"
              ...
              <SelectRow
                ...
                onChange={(value) => updateRibbonDecoration(ribbonIndex, { renderMode: value as TrailRibbonDecorationConfig['renderMode'] })}
              />
              <NumberRow label="startOffset" ariaLabel={`trail-ribbon-${ribbonIndex}-startOffset`} help="相对拖尾起点的偏移" value={ribbon.startOffset} onChange={(value) => updateRibbonDecoration(ribbonIndex, { startOffset: value })} />
              <NumberRow label="endOffset" ariaLabel={`trail-ribbon-${ribbonIndex}-endOffset`} help="相对拖尾末端的偏移" value={ribbon.endOffset} onChange={(value) => updateRibbonDecoration(ribbonIndex, { endOffset: value })} />
              <NumberRow label="thickness" ariaLabel={`trail-ribbon-${ribbonIndex}-thickness`} help="装饰线条厚度系数" value={ribbon.thickness} onChange={(value) => updateRibbonDecoration(ribbonIndex, { thickness: value })} />
              <NumberRow label="alphaScale" ariaLabel={`trail-ribbon-${ribbonIndex}-alphaScale`} help="装饰透明度倍率" value={ribbon.alphaScale} onChange={(value) => updateRibbonDecoration(ribbonIndex, { alphaScale: value })} />
              <NumberRow label="lengthScale" ariaLabel={`trail-ribbon-${ribbonIndex}-lengthScale`} help="按长度渲染时的长度倍率" value={ribbon.lengthScale} onChange={(value) => updateRibbonDecoration(ribbonIndex, { lengthScale: value })} />
              <NumberRow label="nodeCountScale" ariaLabel={`trail-ribbon-${ribbonIndex}-nodeCountScale`} help="按节点数渲染时的节点倍率" value={ribbon.nodeCountScale} onChange={(value) => updateRibbonDecoration(ribbonIndex, { nodeCountScale: value })} />
              <NumberRow label="waveAmplitude" ariaLabel={`trail-ribbon-${ribbonIndex}-waveAmplitude`} help="螺旋幅度" value={ribbon.waveAmplitude} onChange={(value) => updateRibbonDecoration(ribbonIndex, { waveAmplitude: value })} />
              <NumberRow label="waveFrequency" ariaLabel={`trail-ribbon-${ribbonIndex}-waveFrequency`} help="螺旋频率" value={ribbon.waveFrequency} onChange={(value) => updateRibbonDecoration(ribbonIndex, { waveFrequency: value })} />
              <NumberRow label="waveSpeed" ariaLabel={`trail-ribbon-${ribbonIndex}-waveSpeed`} help="螺旋速度" value={ribbon.waveSpeed} onChange={(value) => updateRibbonDecoration(ribbonIndex, { waveSpeed: value })} />
              <SelectRow
                label="waveType"
                ...
                onChange={(value) => updateRibbonDecoration(ribbonIndex, { waveType: value as RibbonWaveType })}
              />
              {ribbon.waveType === 'noise' && (
                <NumberRow label="noiseScale" ariaLabel={`trail-ribbon-${ribbonIndex}-noiseScale`} help="噪声空间频率（越大抖动越密集）" value={ribbon.noiseScale} onChange={(value) => updateRibbonDecoration(ribbonIndex, { noiseScale: value })} />
              )}
              <NumberRow label="blur" ariaLabel={`trail-ribbon-${ribbonIndex}-blur`} help="模糊强度" value={ribbon.blur} onChange={(value) => updateRibbonDecoration(ribbonIndex, { blur: value })} />
              <ColorRow
                label="color"
                help="独立颜色，默认跟随弹体颜色"
              ...
              <BoolRow
                ...
                onChange={(value) => updateRibbonDecoration(ribbonIndex, { colorGradient: { ...ribbon.colorGradient, enabled: value } })}
              />
              <div className="panel-inline-head panel-inline-subhead">
                <span className="param-subgroup-title">Gradient Stops</span>
                <button type="button" onClick={() => addRibbonGradientStop(ribbonIndex)}>Add Stop</button>
              </div>
              {ribbon.colorGradient.stops.map((stop, stopIndex) => (
                <div key={`${ribbon.id}-stop-${stopIndex}`} className="param-row param-row-nested">
                  <label className="param-label" htmlFor={`trail-ribbon-${ribbonIndex}-stop-${stopIndex}`}>
                    <span className="param-name">stop {stopIndex}</span>
                    <span className="param-help">渐变点</span>
                  </label>
                  <div className="param-control param-control-color">
                    <input
                      ...
                      step="0.01"
                      value={stop.offset}
                      onChange={(event) => updateRibbonGradientStop(ribbonIndex, stopIndex, { offset: Number(event.currentTarget.value) })}
                    />
                    <div className="param-control param-control-color">
                        ...
                        <label key={channelIndex} className="param-color-cell">
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              next[channelIndex] = fromColorInputValue(Number(event.currentTarget.value));
                              updateRibbonGradientStop(ribbonIndex, stopIndex, { color: next });
                            }}
                          />
                        </label>
                      ))}
                      <button type="button" onClick={() => removeRibbonGradientStop(ribbonIndex, stopIndex)}>Remove</button>
                    </div>
                  </div>
                </div>
              ))}
    ...
    <section className="param-group">
      <header className="param-group-head">
        ...
        <span className="param-group-meta">
          <span className="muted-chip">{badge}</span>
...
function ColorRow({ label, help, ariaPrefix, value, onChange, onRgbChange }: ColorRowProps) {
  const swatch = `rgb(${Math.round(value[0] * 255)}, ${Math.round(value[1] * 255)}, ${Math.round(value[2] * 255)})`;
    ...
    <div className="param-row">
      ...
      <div className="param-control param-control-color">
        {value.map((channel, index) => (
          <label key={index} className="param-color-cell">
            <span>{channelLabels[index]}</span>
            <input
              ...
              step="1"
              value={toColorInputValue(channel)}
              onChange={(event) => onChange(index, fromColorInputValue(Number(event.currentTarget.value)))}
            />
          </label>
        ))}
      </div>
