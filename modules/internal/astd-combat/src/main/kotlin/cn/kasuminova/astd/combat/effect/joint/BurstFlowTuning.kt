 ...
 *
 * 动机：瞬时爆发（不随时间线性增长/减弱，恒定口径）的时流/机动/备弹恢复三合一系统，
 * 三锚点数值集中在此声明，stats 脚本每次 apply 实时解析（LunaLib 设置变更即时生效）。
 *
 * 数值口径：设计案给定的 v1/v2/v5 三锚点恰为线性步进（时流 +50%/档、机动与备弹逐档恰重合
 * 三锚点插值），登记 LINEAR 无超线性收益（爆发窗口固定 1s，不存在叠乘放大）。
