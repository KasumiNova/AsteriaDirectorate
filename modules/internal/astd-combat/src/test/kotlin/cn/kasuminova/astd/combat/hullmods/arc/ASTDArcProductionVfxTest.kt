...
class ASTDArcProductionVfxTest {
    ...
    fun `production vfx helper uses BoxUtil for link visuals and only fails loudly in automation`() {
        val source = Files.readString(
            RepoLayout.mainSourceFile("combat/hullmods/arc/ASTDArcProductionVfx.kt")!!,
        )
    ...
    fun `plasma arch recoil accumulator arcs use wider slower arcs with path lens flares`() {
        val source = Files.readString(
            RepoLayout.mainSourceFile("combat/hullmods/arc/ASTDIonizedRecoilAccumulatorHullMod.kt")!!,
        )
    ...
    fun `production runtime call sites route acceptance vfx through helper`() {
        ...
        callSites.forEach { (file, helperCall) ->
            val source = Files.readString(
                RepoLayout.mainSourceFile(
                    if (file.contains("SystemStats")) "combat/shipsystems/$file" else "combat/hullmods/arc/$file",
                )!!,
            )
        ...
        val tactical = Files.readString(
            RepoLayout.mainSourceFile("combat/hullmods/arc/ASTDArcSharedTacticalNetworkHullMod.kt")!!,
        )
        val pursuit = Files.readString(
            RepoLayout.mainSourceFile("combat/hullmods/arc/ASTDDistributedPursuitNetworkHullMod.kt")!!,
        )
    ...
    fun `arc production range and fire control modifiers cover strict non missile wording`() {
        val fireControl = Files.readString(
            RepoLayout.mainSourceFile("combat/hullmods/arc/ASTDArcAdvancedFireControlHullMod.kt")!!,
        )
        assertTrue(
        ...
        val targeting = Files.readString(
            RepoLayout.mainSourceFile("combat/hullmods/arc/ASTDArcAdvancedTargetingSystemHullMod.kt")!!,
        )
        assertTrue(
    ...
    fun `arc shared networks clean target modifiers and honor system range stats`() {
        ...
        val fluxNetwork = Files.readString(
            RepoLayout.mainSourceFile("combat/shipsystems/ASTDArcSharedFluxNetworkSystemStats.kt")!!,
        )
        assertTrue(
    ...
    fun `limit temporal thruster player time compensation is owned by the active player ship`() {
        val source = Files.readString(
            RepoLayout.mainSourceFile("combat/shipsystems/ASTDLimitTemporalThrusterSystemStats.kt")!!,
        )
    ...
    fun `arc production tooltip evidence validates renderer block keys`() {
        val plugin = Files.readString(
            RepoLayout.mainSourceFile("combat/effect/generic/ASTDAutomationCombatPlugin.kt")!!,
        )
    ...
    fun `radiation belt system visual has independent telemetry evidence`() {
        val vfx = Files.readString(RepoLayout.mainSourceFile("combat/hullmods/arc/ASTDArcProductionVfx.kt")!!)
        val system = Files.readString(RepoLayout.mainSourceFile("combat/shipsystems/ASTDLimitTemporalThrusterSystemStats.kt")!!)
